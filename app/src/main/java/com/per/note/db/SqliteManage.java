...
public class SqliteManage {
    ...
    private SqliteManage(Context context) {
        ...
        db.execSQL("create table if not exists inout(_id INTEGER PRIMARY KEY AUTOINCREMENT,year " +
                ...
                "inout varchar(20),class varchar(20),count varchar(20),time varchar(20),other varchar(60))");//创建流水表
        db.execSQL("create table if not exists count(_id INTEGER PRIMARY KEY AUTOINCREMENT,count varchar(20),money varchar(20))");//创建账本表
        db.execSQL("create table if not exists time(_id INTEGER PRIMARY KEY AUTOINCREMENT,time varchar(20),value varchar(20))");//创建时间表
        db.close();
    }
